...
| メタデータ DB | SQLite (Prisma) |
| チャート/ラベル DB | MongoDB |
| インフラ | Docker Compose（MongoDB） |
| Storybook | 8.x |
...
1. **スキーマ拡張**（[05](./05-schema-changes.md)）— FX/株両対応
2. **データ自動収集**（[02](./02-data-collection.md)）— Yahoo Finance ベース
3. **マルチアセット UI**（[04](./04-multi-asset-support.md)）— 銘柄選択・株用時間足
4. **自動ラベリング**（[03](./03-auto-labeling.md)）— ルールベース → LLM 補助
